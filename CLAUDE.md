...
| Page zoom | 100 on entry; `−` down to 30 and `+` up to 300, each greying out at its end. **The check is that it RE-LAYS-OUT**, not that it magnifies: at 50 % the whole session list must fit across with no sideways scrolling, and at 200 % everything must be twice the size and still fit. A region growing under a fixed layout is a pinch, and wrong. Measured on the DT50 at 50 % and 200 % against 1.20.0 |
...
| Desktop mode | **off** on entry, every time. On, the 1280 px layout scaled to fit, and the button must look pressed while it is. Leave and come back: off again |
