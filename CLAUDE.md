...
| Page zoom | 100 on entry; `−` down to 30 and `+` up to 300, each greying out at its end. **The check is that it RE-LAYS-OUT**, not that it magnifies: at 50 % more page must fit across, at 200 % everything must be twice the size, and at **no** setting may the page scroll sideways. A region growing under a fixed layout is a pinch, and the bar is not the pinch. **Both modes, because there is only one rule**: the switch picks the width at 100 and the zoom divides it. Measured on the DT50 against 1.21.0 at 50 % and 200 % with the switch off, and 150 % and 200 % with it on |
